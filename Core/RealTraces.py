...
class RealTraces():
    ...
    def load_traces_from_csv(filename, amount_of_traces, dtype=np.float32):
        data = []
        data_counter = 0
            ...
            for row in alignment:
                for genome in range(3,len(row)-11+1,11):
                    if row[genome] in alignment_per_genome.keys():
                        dict_row  = dict(zip(['tested_trace_id','xcorr','stretch','pvalue','direction','match_len'],[[int(row[0])],[float(row[genome+1])],[float(row[genome+2])],[float(row[genome+3])],[row[genome+4]],[int(row[genome+5])]]))
    ...
    def normalize_traces_local(self,window):
    ...
    def pick_traces_subset(self,genome):
        self.trace_subset = []
    ...
    def prepare_input_size(self,sz):
        ...
        for trace in self.trace_subset:
            startind = np.random.randint(0,len(trace)-sz)
    ...
    def plot_traces(self):
        ...
        plt.plot(self.normalized_traces[5])
